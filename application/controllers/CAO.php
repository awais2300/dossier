...
class CAO extends CI_Controller
{
    ...
    public function view_semester_result()
    {
        ...
        $this->load->view('cao/add_semester_result', false);
    }
    ...
    public function save_cadet_semester_result()
    {
        ...
        if (!empty($insert) || !empty($update)) {
            $this->session->set_flashdata('success', 'Data Submitted successfully');
            redirect('CAO/view_semester_result');
        } else {
            $this->session->set_flashdata('failure', 'Something went wrong, try again.');
            redirect('CAO/view_semester_result');
        }
    ...
    {
        if ($this->session->has_userdata('user_id')) {
            $this->load->view('cao/add_proficieny_games');
        }
    }
    ...
    public function save_proficiency_games()
    {
        if ($this->input->post()) {
            ...
            if (!empty($insert)) {
                ...
                redirect('CAO/view_proficieny_games');
            } else {
                $this->session->set_flashdata('failure', 'Something went wrong, try again.');
                redirect('CAO/view_proficieny_games');
            }
    ...
    public function proficiency_games_records_report($oc_no = NULL)
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $data['test_records'] = $this->db->get()->result_array();
            $html = $this->load->view('cao/proficiency_games_report', $data, TRUE); //$graph, TRUE);
